...
import { log, progress } from "@serverless/utils/log";
...
export class StaticWebsite extends AwsConstruct {
    ...
    async uploadWebsiteCommand(): Promise<void> {
        if (isV3) {
            ...
            legacyLog(`Deploying the static website '${this.id}'`);
        }
        ...
        if (domain !== undefined) {
            if (isV3) {
                log();
                log.success(`Deployed https://${domain} ${chalk.gray(`(${fileChangeCount} files changed)`)}`);
            } else {
                legacyLog(`Deployed https://${domain} ${chalk.gray(`(${fileChangeCount} files changed)`)}`);
            }
        }
    }
    ...
    private async uploadWebsite(): Promise<number> {
        ...
        if (isV3) {
            ...
            log.verbose(`Uploading directory '${this.configuration.path}' to bucket '${bucketName}'`);
        } else {
            legacyLog(`Uploading directory '${this.configuration.path}' to bucket '${bucketName}'`);
        }
        ...
        if (hasChanges) {
            if (uploadProgress) {
                ...
                legacyLog(`Clearing CloudFront DNS cache`);
            }
