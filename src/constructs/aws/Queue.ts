...
import type { Progress } from "@serverless/utils/log";
...
export class Queue extends AwsConstruct {
    ...
    async listDlq(): Promise<void> {
        ...
        if (messages.length === 0) {
            if (progressV2) {
                ...
                log.success("No failed messages found in the dead letter queue");
            }
        ...
        if (progressV2) {
            ...
            log(`${messages.length} messages found in the dead letter queue:`);
            log();
        }
        for (const message of messages) {
            if (isV3) {
                writeText(chalk.gray(`Message #${message.MessageId ?? "?"}`));
                writeText(this.formatMessageBody(message.Body ?? ""));
                writeText();
            } else {
        ...
        if (isV3) {
            log(`Run ${retryCommand} to retry all messages, or ${purgeCommand} to delete those messages forever.`);
        } else {
            console.log(
    ...
    async purgeDlq(): Promise<void> {
        ...
        if (progressV3) {
            progressV3.remove();
            log.success("The dead letter queue has been purged, failed messages are gone 🙈");
        } else if (progressV2) {
    ...
    async retryDlq(): Promise<void> {
        ...
        if (progressV3) {
            progressV3.remove();
            log.success(`${totalMessagesRetried} failed message(s) moved to the main queue to be retried 💪`);
        } else if (progressV2) {
    ...
    async sendMessage(options: CliOptions): Promise<void> {
        ...
        if (isV3) {
            log.success("Message sent to SQS");
        }
    }
    ...
    displayLogs(options: CliOptions): void {
        ...
        if (isV3) {
            log(chalk.gray(`serverless ${args.join(" ")}`));
        } else {
